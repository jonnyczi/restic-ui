...
**How restic is driven.** There is no restic library — `internal/restic` shells out to a bundled, pinned `restic` binary and parses `--json` output. The repository location and password travel via environment variables (`RESTIC_REPOSITORY`, `RESTIC_PASSWORD`), never argv (visible in `ps`). restic's cache is pinned to `DATA_DIR/cache` via `RESTIC_CACHE_DIR`, and SFTP host keys are TOFU-persisted at `DATA_DIR/ssh/known_hosts` (the entrypoint also sets the runtime user's home to `$DATA_DIR` via usermod — su-exec resets `HOME` to the passwd entry, so exporting it wouldn't survive). restic exit codes are mapped to friendly errors (10 = not initialized, 11 = locked, 12 = wrong password); backup exit code 3 becomes a `warning` status (snapshot created, some files unreadable).
